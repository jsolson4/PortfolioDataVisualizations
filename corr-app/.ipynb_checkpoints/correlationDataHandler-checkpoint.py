...
class correlationDataHandler:
    ...
    def correlation_links(self, period) -> pd.DataFrame:
        """
    ...
    def correlation_nodes(self, df) -> pd.DataFrame:
        """
        Get the average correlations 
        """
        
        df = df.groupby(['source'], as_index = False)['Correlation'].mean().round(3).rename(columns={'source':'name'}).drop_duplicates()
